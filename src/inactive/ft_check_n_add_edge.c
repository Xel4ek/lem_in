...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_check_n_add_edge.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ayooden <marvin@42.fr>                     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/03/03 18:41:31 by ayooden           #+#    #+#             */
/*   Updated: 2020/03/03 18:41:33 by ayooden          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int 	ft_check_repeat(t_hash_old **edge_hash, t_vertex *v1, t_vertex *v2)
{
	int		res;
	char	*str1;
	char	*str2;
	...
	res = 0;
	if (ft_find_edge_in_hash(edge_hash, str1, ft_hash_old(str1, HASH_SIZE)) && \
		!((v1->id == 0 && v2->id == -2) || (v1->id == -2 && v2->id == 0)))
		res = 1;
	if (ft_find_edge_in_hash(edge_hash, str2, ft_hash_old(str2, HASH_SIZE)) && \
		!((v1->id == 0 && v2->id == -2) || (v1->id == -2 && v2->id == 0)))
		res = 1;
	ft_memdel((void **)&str2);
	if (!res)
		ft_add_to_hash(edge_hash, (void *)str1, (void *)str1, \
        ft_hash_old(str1, HASH_SIZE));
	else
		ft_memdel((void **)&str1);
	return (res);
}
...
int			ft_check_n_add_edge(t_edgename *enames, t_hash_old **edge_hash, \
			t_hash_old **hashtab)
{
	t_vertex	*vertex1;
	t_vertex	*vertex2;
